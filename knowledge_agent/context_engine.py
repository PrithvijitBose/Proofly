...
class ContextEngine:
    ...
    def extract_pr_references(text: str, current_issue_number: Optional[int] = None) -> List[int]:
        ...
        if current_issue_number is not None:
            prs = [p for p in prs if p != current_issue_number]
        return prs
    ...
    def categorize_comments(comments: List[Dict[str, Any]], issue_author: str = "") -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for c in (comments or []):
            ...
            is_maintainer = (
                author.lower() == (issue_author or "").lower() or
    ...
    def fetch_linked_prs(
        ...
        for num in pr_numbers:
            ...
            if access_token:
                pr_data = GitHubClient.fetch_pull_request(access_token, owner, repo, num)
                if pr_data:
                    files_data = GitHubClient.fetch_pr_files(access_token, owner, repo, num)
                    file_names = [
                        f.get("filename")
    ...
    def build_structured_context(
        ...
        issue_author = (issue.get("user") or {}).get("login", "Maintainer") if isinstance(issue.get("user"), dict) else "Maintainer"

        # 1. Combine all text to extract PR references
        combined_text = f"{issue_title}\n{issue_body}\n" + "\n".join(
            [(c.get("body") or "") for c in (comments or []) if isinstance(c, dict)]
        )
        ...
        if fetched_files is None:
            file_paths = RelationshipExtractor.extract_referenced_files(combined_text)
            fetched_files = {}
            if access_token:
                for path in file_paths:
                    content = GitHubClient.fetch_file_content(access_token, owner, repo, path)
                    if content:
    ...
    def format_evidence_prompt(
        ...
        lines.append(f"Description:\n{issue.get('body') or '*No body content*'}\n")

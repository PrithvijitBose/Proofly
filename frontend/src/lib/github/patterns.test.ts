...
/** 17-record fixture covering commits, PRs, issues, languages and events. */
function fixture(): EvidenceRecord[] {
  return [
    // Commits — 2023: repo-a; 2024: repo-a ×3 + repo-b ×2; 2025: repo-a ×2 + repo-b ×2
    ...
    rec({ id: "c5", repoFullName: REPO_A, date: "2025-03-10T00:00:00Z", title: "Optimize bundle size", meta: { message: "Optimize bundle size" } }),
    ...
    rec({ id: "c9", repoFullName: REPO_B, date: "2025-07-02T00:00:00Z", title: "Deploy data pipeline", meta: { message: "Deploy data pipeline" } }),
    rec({ id: "c10", repoFullName: REPO_B, date: "2024-03-15T00:00:00Z", title: "Refactor error handling", meta: { message: "Refactor error handling" } }),
    // Pull requests
    ...
    rec({ id: "p3", source: "pull_request", repoFullName: REPO_B, date: "2025-03-20T00:00:00Z", title: "Add retry logic", meta: { number: 3, state: "closed", merged: true } }),
    // Issue
...
describe("analyzePatterns", () => {
  ...
  it("derives timeline facts from activity records", () => {
    ...
    expect(mostActive!.statement).toContain("5");
    // Every id in the fact belongs to a 2024 commit.
    expect(mostActive!.evidenceIds.sort()).toEqual(["c10", "c2", "c3", "c6", "c8"]);
  });
  ...
  it("uses language byte counts + commit dates for evolution, not repo metadata", () => {
    ...
    // 2025: repo-a (2 TypeScript commits) ties repo-b (2 Python commits);
    // deterministic alphabetical tie-break → Python.
    const year2025 = evolution.find((f) => f.label === "2025");
    expect(year2025!.statement).toContain("Python");
    expect(year2025!.evidenceIds).toContain("l2");
  ...
  it("computes cadence per quarter, busiest quarter and longest streak", () => {
    ...
    expect(q1_2024!.evidenceIds.sort()).toEqual(["c10", "c2", "p1"]);
  ...
  it("is fully deterministic: same input, same output", () => {
    ...
    // ids are unique and ordered identically across runs
    expect(new Set(second.map((f) => f.id)).size).toBe(second.length);
